...
function discreteSI_model(S, I, R)
    ...
    for n = 1:30
        push!(S, (S[n] * (1 - produto * I[n])) + beta * delta_t * (N * (1 - p) - S[n])) # armazena o novo valor no vetor de suscetíveis
        ...
        push!(R, R[n] * (1 - beta * delta_t) + gamma * delta_t * I[n] + beta * p * delta_t * N) 
    end
    println(S)
    println(I)
    println(R)
...
function main()
    ...
    elseif p <= 0 || p > 1
        println("p deve estar entre zero e 1")
